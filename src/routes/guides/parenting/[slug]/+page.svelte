<!-- src/routes/guides/parenting/[slug]/+page.svelte -->
<script lang="ts">
  ...
  } from '$lib/stores/parentingGuideStore';
  ...
  // Access translations
  $: t = $parentingGuideStore.translations[currentLanguage];
  ...
  $: currentItem = $parentingGuideStore.currentItem;
  ...
  async function loadContent(itemPath) {
    ...
    try {
      ...
      const pathsToTry = [
        `${base}/content/guides/parenting/${filePath}.md`,
        `/content/guides/parenting/${filePath}.md`
      ];
  ...
  function navigateToItem(path) {
    goto(`${base}/guides/parenting/${path}`, {
      replaceState: false
    });
  }
</script>
...
<svelte:head>
  <title>{itemTitle ? `${itemTitle} | Spiralize` : 'Parenting Guide | Spiralize'}</title>
</svelte:head>
    ...
    <div in:fade>
      ...
      <nav class="flex mb-6 text-sm" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
          <li class="inline-flex items-center">
            <a href="{base}/guides/parenting" class="text-gray-500 hover:text-teal-600">
              {t.tableOfContents}
            </a>
          </li>
          ...
          {#if parentItem && parentItem.type === 'section'}
            <li>
              <div class="flex items-center">
                ...
                <a href="{base}/guides/parenting/{parentItem.path}" class="ml-1 text-gray-500 hover:text-teal-600">
      ...
      <div class="mb-10">
        ...
        {:else if currentItem?.type === 'subsection' && parentItem}
          <div class="text-sm text-teal-600 mb-2">
            <a href="{base}/guides/parenting/{parentItem.path}" class="hover:underline">
            ...
            <nav>
              <ul class="space-y-1 border-l border-gray-200 pl-4">
                {#each mainSections as section}
                  <li>
                    <a 
                      href="{base}/guides/parenting/{section.path}"
                    ...
                    {#if currentItem && ((currentItem.id === section.id) || (currentItem.parentId === section.id)) && getSubsections(section.id).length > 0}
                      <ul class="mt-1 ml-4 space-y-1 border-l border-gray-200 pl-3">
                        {#each getSubsections(section.id) as subsection}
                          <li>
                            <a 
                              href="{base}/guides/parenting/{subsection.path}"
              ...
              <div class="mt-8">
                ...
                <ul class="space-y-1 border-l border-gray-200 pl-4">
                  {#each resources as resource}
                    <li>
                      <a 
                        href="{base}/guides/parenting/{resource.path}"
        ...
        <div class="flex-1">
                  ...
                  >
                    <h3 class="font-medium mb-1">
                      ...
                      {:else if resource.isWorksheet}
                        <span class="text-sm bg-teal-100 text-teal-700 px-2 py-0.5 rounded-full mr-2">
                          {t.worksheetText}
                        </span>
                      {/if}
                      {resource.title[currentLanguage]}
          ...
          <div class="flex justify-between mt-16 pt-8 border-t border-gray-200">
            {#if adjacentItems.previous}
              <a 
                href="{base}/guides/parenting/{adjacentItems.previous.path}" 
            ...
            {#if adjacentItems.next}
              <a 
                href="{base}/guides/parenting/{adjacentItems.next.path}" 
                class="flex items-center text-teal-600 hover:text-teal-800"
              >
