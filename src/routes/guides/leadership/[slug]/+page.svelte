<!-- src/routes/guides/leadership/[slug]/+page.svelte -->
<script lang="ts">
  ...
  $: currentItem = $leadershipGuideStore.currentItem;
  ...
  // Is this a special resource type (tool, worksheet, etc)?
  ...
  function formItemTitle(item) {
    ...
    } else if (item.isExercise) {
      return `${t.exerciseText}: ${item.title[currentLanguage]}`;
  ...
  async function loadContent(itemPath) {
    ...
    try {
      ...
      // Get the filePath from the item (using the new property we added)
      ...
      const pathsToTry = [
        `${base}/content/guides/leadership/${filePath}.md`,
        `/content/guides/leadership/${filePath}.md`
      ];
  ...
  function navigateToItem(path) {
    goto(`${base}/guides/leadership/${path}`, {
      replaceState: false
    });
  }
</script>
...
<svelte:head>
  <title>{itemTitle ? `${itemTitle} | Spiralize` : 'Leadership Guide | Spiralize'}</title>
</svelte:head>
    ...
    <div in:fade>
      ...
      <nav class="flex mb-6 text-sm" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
          <li class="inline-flex items-center">
            <a href="{base}/guides/leadership" class="text-gray-500 hover:text-purple-600">
              {t.tableOfContents}
            </a>
          </li>
          ...
          {#if parentItem && parentItem.type === 'section'}
            <li>
              <div class="flex items-center">
                ...
                <a href="{base}/guides/leadership/{parentItem.path}" class="ml-1 text-gray-500 hover:text-purple-600">
      ...
      <div class="mb-10">
        ...
        {:else if currentItem?.type === 'subsection' && parentItem}
          <div class="text-sm text-purple-600 mb-2">
            <a href="{base}/guides/leadership/{parentItem.path}" class="hover:underline">
        ...
        {:else if isResourceItem && parentItem}
          ...
          <div class="text-sm text-purple-600 mb-2">
            <a href="{base}/guides/leadership/{parentItem.path}" class="hover:underline">
              {t.sectionText} {parentItem.section}
            </a> / {currentItem?.isTool ? t.toolText : currentItem?.isExercise ? t.exerciseText : currentItem?.isTemplate ? t.templateText : currentItem?.isWorksheet ? t.worksheetText : ''}
          </div>
        {/if}
            ...
            <nav>
              <ul class="space-y-1 border-l border-gray-200 pl-4">
                {#each mainSections as section}
                  <li>
                    <a 
                      href="{base}/guides/leadership/{section.path}"
                      class="block py-1 hover:text-purple-600 transition-colors {currentItem?.id === section.id || (currentItem?.parentId === section.id) ? 'font-semibold text-purple-600 border-l-2 border-purple-600 -ml-[17px] pl-[15px]' : 'text-gray-600'}"
                    >
                    ...
                    {#if currentItem && ((currentItem.id === section.id) || (currentItem.parentId === section.id)) && getSubsections(section.id).length > 0}
                      <ul class="mt-1 ml-4 space-y-1 border-l border-gray-200 pl-3">
                        {#each getSubsections(section.id) as subsection}
                          <li>
                            <a 
                              href="{base}/guides/leadership/{subsection.path}"
                              class="block py-1 text-sm hover:text-purple-600 transition-colors {currentItem?.id === subsection.id ? 'font-semibold text-purple-600 border-l-2 border-purple-600 -ml-[17px] pl-[15px]' : 'text-gray-600'}"
                            >
              ...
              <div class="mt-8">
                ...
                <ul class="space-y-1 border-l border-gray-200 pl-4">
                  {#each resources as resource}
                    <li>
                      <a 
                        href="{base}/guides/leadership/{resource.path}"
                        class="block py-1 text-sm hover:text-purple-600 transition-colors {currentItem?.id === resource.id ? 'font-semibold text-purple-600 border-l-2 border-purple-600 -ml-[17px] pl-[15px]' : 'text-gray-600'}"
                      >
                        <span>
                          {#if resource.isTool}
                          ...
                          {:else if resource.isExercise}
                            <span class="bg-green-100 text-green-700 text-xs rounded-full px-2 py-0.5 mr-1">
                              {t.exerciseText}
                            </span>
                          {:else if resource.isTemplate || resource.isWorksheet}
                            <span class="bg-amber-100 text-amber-700 text-xs rounded-full px-2 py-0.5 mr-1">
                              {resource.isTemplate ? t.templateText : t.worksheetText}
                            </span>
                          {/if}
                        </span>
        ...
        <div class="flex-1">
            ...
            <div class="mt-12 border-t border-gray-200 pt-8">
              ...
              <div class="grid gap-4 md:grid-cols-2">
                {#each subsections as subsection}
                  <a 
                    href="{base}/guides/leadership/{subsection.path}"
                    class="p-4 bg-white rounded-lg border border-gray-200 hover:border-purple-400 hover:shadow-md transition-all flex flex-col"
                  >
          ...
          {#if (currentItem?.type === 'section' || currentItem?.type === 'subsection') && resources.length > 0}
            <div class="mt-12 p-6 bg-purple-50 rounded-lg">
              <h2 class="text-xl font-semibold mb-4 text-purple-700">{t.resources}</h2>
              <div class="grid gap-4 md:grid-cols-2">
                {#each resources as resource}
                  <a 
                    href="{base}/guides/leadership/{resource.path}"
          ...
          <div class="flex justify-between mt-16 pt-8 border-t border-gray-200">
            {#if adjacentItems.previous}
              <a 
                href="{base}/guides/leadership/{adjacentItems.previous.path}" 
            ...
            {#if adjacentItems.next}
              <a 
                href="{base}/guides/leadership/{adjacentItems.next.path}" 
                class="flex items-center text-purple-600 hover:text-purple-800"
              >
